...
//   GET  /auto-open-web/config → { ok, timeout, appWindow,
//                                   windowKind, browserPath, writable }
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function AutoOpenSettingsPage() {
      ...
      const [config, setConfig] = React.useState(null) // { timeout, appWindow, windowKind, browserPath, writable } | null
      const [loadError, setLoadError] = React.useState('')
      const [reloadTick, setReloadTick] = React.useState(0)
      const [draft, setDraft] = React.useState({ timeout: 10000, appWindow: true, windowKind: 'webview2', browserPath: '' })
      ...
      React.useEffect(() => {
        ...
        fetch('/auto-open-web/config')
          ...
          .then((result) => {
            ...
            setConfig({
              timeout: typeof result.timeout === 'number' ? result.timeout : 10000,
      ...
      const saved = config !== null
        ? { timeout: config.timeout, appWindow: config.appWindow, windowKind: config.windowKind, browserPath: config.browserPath, exitOnWindowClose: config.exitOnWindowClose }
        : null
      ...
      // [诊断] 每次渲染输出关键状态,便于定位"保存按钮置灰"
      React.useEffect(() => {
        console.log('[auto-open-web] render-state', JSON.stringify({
          blocked: saved === null || !dirty || invalid || busy,
          dirty,
          invalid,
          busy,
          picking,
          hasSaved: saved !== null,
          hasSetPath: setPath !== null,
          writable,
          loadError,
          draft,
          saved,
          ...
          .then((result) => {
            if (result !== null && result !== undefined && result.ok === true) {
              setConfig((prev) => prev !== null ? {
                timeout: typeof result.timeout === 'number' ? result.timeout : prev.timeout,
                appWindow: result.appWindow !== false,
                windowKind: result.windowKind === 'browser' ? 'browser' : 'webview2',
                browserPath: typeof result.browserPath === 'string' ? result.browserPath : '',
                exitOnWindowClose: result.exitOnWindowClose === true,
                ...
                : React.createElement(React.Fragment, null,
                    ...
                    React.createElement('div', { className: 'At1oFq_field' },
                      React.createElement('div', { className: 'At1oFq_head' },
                        React.createElement('label', { className: 'At1oFq_label', htmlFor: 'auto-open-web-timeout' }, '端口等待超时(timeout)')
                      ),
                      React.createElement(ProxyInput, {
                        id: 'auto-open-web-timeout',
                        type: 'number',
                        min: 0,
                        step: 1000,
                        className: 'aow-timeout',
                        value: String(draft.timeout),
                        disabled: !writable,
                        onChange: (event) => {
                          const raw = event.target.value
                          const num = raw === '' ? 0 : Number(raw)
                          const next = applySetPath(draft, ['timeout'], Number.isFinite(num) ? num : 0)
                          console.log('[auto-open-web] change timeout ->', JSON.stringify(next.timeout), 'dirty:', next.timeout !== saved?.timeout)
                          setDraft(next)
                        }
                      }),
                      React.createElement('p', { className: 'At1oFq_hint' }, '等待 webServer 监听端口公布的最大毫秒数;正常启动毫秒级即可拿到端口,超时仅在异常时兜底')
                    ),
                    // ---- 独立应用窗口 ----
                    React.createElement('div', { className: 'At1oFq_field' },
                      React.createElement('div', { className: 'At1oFq_head' },
                      ...
                      React.createElement('div', { className: 'aow-row' },
                        React.createElement(ProxyInput, {
                          ...
                          onChange: (event) => {
                            ...
                            console.log('[auto-open-web] change browserPath ->', JSON.stringify(next.browserPath), 'dirty:', next.browserPath !== saved?.browserPath)
                            setDraft(next)
                          }
                        }),
